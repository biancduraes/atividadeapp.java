// Importações necessárias para trabalhar com servidor HTTP e arquivos
...
public class App {
    ...
    static final int PORT = 8080;       // porta onde o servidor vai rodar
    static final String CSV = "data_tasks.csv"; // nome do arquivo que salva as tarefas
    static final int MAX = 5000;        // número máximo de tarefas

    // Arrays para armazenar as tarefas em memória
    ...
    static long[] criados = new long[MAX];  // data de criação
    static int n = 0; // quantidade atual de tarefas

    public static void main(String[] args) throws Exception {
        // Carrega tarefas já salvas no CSV
        carregar();

        // Cria o servidor
        HttpServer server = HttpServer.create(new InetSocketAddress(PORT), 0);

        // Rota principal (abre o HTML do kanban)
        server.createContext("/", new RootHandler());

        // Rota da API para manipular tarefas
    ...
    // cLASSE QUE SERVE O HTML PRINCIPAL 
    ...
    // CLASSE QUE SERVE A API 
    static class ApiTasksHandler implements HttpHandler {
        @Override public void handle(HttpExchange ex) throws IOException {
            String method = ex.getRequestMethod(); // método HTTP (GET, POST, PATCH, DELETE)
            URI uri = ex.getRequestURI(); // pega a URL chamada
            String path = uri.getPath();  // pega o caminho da URL
            ...
            try {
                ...
                // ATUALIZAR STATUS DA TAREFA (mover entre TODO, DOING e DONE)
                ...
                if ("DELETE".equals(method) && path.startsWith("/api/tasks/")) {
                    ...
                    // reorganiza o array depois de apagar
                    for (int k = i; k < n - 1; k++) {
                        ids[k] = ids[k+1]; 
    ...
    // HTML DO FRONT-END 
    // esse HTML monta a página do kanban
...
<script>
// Aqui é o JavaScript que fala com a API e monta o kanban
const API = "/api/tasks";

// Lista tarefas e mostra na tela
...
// Quando enviar o formulário cria uma tarefa nova
...
""";
    ...
    // Pega valor de um campo no JSON (bem simples, não cobre todos os casos)
    ...
    // Funções auxiliares para enviar resposta HTTP
